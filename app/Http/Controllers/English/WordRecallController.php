...
use App\Services\English\WordRecallDataService;
use App\Services\English\WordRecallService;
use Illuminate\Http\Request;
use Inertia\Inertia;

class WordRecallController extends Controller
{
    protected WordRecallDataService $dataService;
    protected WordRecallService $gameService;

    public function __construct(WordRecallDataService $dataService, WordRecallService $gameService)
    {
    ...
    public function index()
    {
        ...
        $memoryMasterRanks = $this->dataService->getMemoryMasterRanks();

        return Inertia::render('English/Games/WordRecall/Index', [
            'levels' => $levels,
            'stats' => $stats,
            'config' => $config,
            ...
            'memoryMasterRanks' => $memoryMasterRanks,
        ]);
    }
    ...
    public function play(int $level)
    {
        ...
        if (!$levelData) {
            return redirect()->route('student.english.games.word-recall.index')
        ...
        return Inertia::render('English/Games/WordRecall/Play', [
            'level' => $levelData,
            'config' => $config,
            'gameModes' => $gameModes,
            'powerups' => $powerups,
    ...
    public function getNextRound(Request $request)
    {
        ...
        try {
            $result = $this->gameService->getNextRound($request->input('session_id'));
    ...
    public function checkRound(Request $request)
    {
        ...
        try {
            $result = $this->gameService->checkRound(
