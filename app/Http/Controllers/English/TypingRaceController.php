...
use App\Services\English\TypingRaceDataService;
use App\Services\English\TypingRaceService;
...
class TypingRaceController extends Controller
{
    ...
    protected TypingRaceService $gameService;

    public function __construct(TypingRaceDataService $dataService, TypingRaceService $gameService)
    {
    ...
    public function index()
    {
        ...
        $typingMasterRanks = $this->dataService->getTypingMasterRanks();

        return Inertia::render('English/Games/TypingRace/Index', [
            'levels' => $levels,
            'stats' => $stats,
            'config' => $config,
            ...
            'typingMasterRanks' => $typingMasterRanks,
        ]);
    }
    ...
    public function play(int $level)
    {
        ...
        if (!$levelData) {
            return redirect()->route('student.english.games.typing-race.index')
        ...
        return Inertia::render('English/Games/TypingRace/Play', [
            'level' => $levelData,
            'config' => $config,
            'gameModes' => $gameModes,
            'powerups' => $powerups,
    ...
    public function submitText(Request $request)
    {
        ...
        try {
            $result = $this->gameService->submitText(
                $request->input('session_id'),
                $request->input('typed_text'),
                $request->input('time')
            );
