...
const goBackToLessons = () => {
    console.log('goBackToLessons called')
    try {
        router.visit(route('student.english.levels'), {
...
const exitLesson = () => {
    console.log('exitLesson called, phase:', phase.value)
    if (phase.value === 'complete' || phase.value === 'failed') {
        goBackToLessons()
    } else if (confirm(t.lesson.exitConfirm)) {
        goBackToLessons()
    }
}
...
<template>
    <EnglishLayout :showNavbar="false">
        <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            <!-- Progress Bar -->
            <LessonProgress
                v-if="phase !== 'complete' && phase !== 'failed'"
                :current="currentStep"
                :total="totalSteps"
            ...
            <main class="max-w-3xl mx-auto px-4 py-6 pt-20">
                ...
                <LessonComplete
                    ...
                    :lesson="lesson"
                    :rewards="completionData || calculateRewards()"
                    :stats="{
                        correct: correctCount,
                        incorrect: incorrectCount,
                        maxStreak: maxStreak,
