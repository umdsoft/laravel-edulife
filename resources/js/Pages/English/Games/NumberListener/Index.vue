...
const currentRank = computed(() => {
    if (!props.numberMasterRanks.length) return null

    let current = props.numberMasterRanks[0]
    for (const rank of props.numberMasterRanks) {
...
const tips = [
    { icon: '🎧', text: "Raqamni diqqat bilan tinglang va to'g'ri yozing!" },
    { icon: '⭐', text: "3 ta yulduz olish uchun 95%+ aniqlik kerak" },
    { icon: '🔊', text: "Ovozni qayta eshitish uchun tugmani bosing" },
]

const startLevel = (level) => {
    if (level.unlocked) {
        router.visit(route('student.english.games.number-listener.play', { level: level.level_number }))
    }
}
</script>

<template>
    <Head title="Number Listener - Levels" />
    ...
    <StudentLayout>
        <div class="min-h-screen pb-8">
            ...
            <div class="relative rounded-3xl p-6 md:p-8 mb-8 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-500 animate-gradient"></div>

                <!-- Mesh Pattern Overlay -->
                <div class="absolute inset-0 opacity-30" style="background-image: url(&quot;data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.15'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E&quot;)"></div>

                <!-- Glow Effects -->
                <div class="absolute -top-40 -right-40 w-96 h-96 bg-cyan-400/20 rounded-full blur-3xl animate-pulse-slow"></div>
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-cyan-400 via-blue-400 to-indigo-500 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shadow-2xl shadow-cyan-500/40 transform hover:scale-105 hover:rotate-3 transition-all duration-300">
                                    🔢
                                </div>
                            </div>
                            <div>
                                <h1 class="text-3xl md:text-5xl font-black text-white mb-1 tracking-tight">
                                    Number <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400">Listener</span>
                                </h1>
                                <p class="text-white/80 text-sm md:text-lg font-medium">
                                    Raqamlarni <span class="text-cyan-300 font-bold">eshiting</span> va <span class="text-green-400 font-bold">yozing</span>!
                                </p>
                            </div>
                        </div>
                    ...
                    <!-- Game Modes Cards -->
                    <div v-if="gameModes.length" class="mt-8 grid grid-cols-2 lg:grid-cols-5 gap-3">
                        <div
                            ...
                            :class="[
                                ...
                                index === 4 ? 'hover:shadow-lg hover:shadow-cyan-500/20' : '',
                            ]"
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    ...
                                    :class="[
                                        ...
                                        index === 4 ? 'bg-gradient-to-br from-cyan-400 to-cyan-600' : '',
                                    ]"
                                >
                                    {{ mode.icon }}
                                </div>
            ...
            <!-- Stats Cards Section (White cards with dark mode support) -->
            <div class="mb-8 grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all">
                    ...
                    <div class="text-gray-600 dark:text-gray-400 text-sm font-medium mt-1">O'yinlar</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all">
                    <div class="text-3xl font-black text-green-600 dark:text-green-400">{{ stats.accuracy || 0 }}%</div>
                    <div class="text-gray-600 dark:text-gray-400 text-sm font-medium mt-1">Aniqlik</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all">
                    <div class="text-3xl font-black text-yellow-600 dark:text-yellow-400">{{ stats.best_streak || 0 }}</div>
                    <div class="text-gray-600 dark:text-gray-400 text-sm font-medium mt-1">Eng yaxshi seriya</div>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all">
                    <div class="text-3xl font-black text-blue-600 dark:text-blue-400">{{ currentRank?.name || 'Yangi' }}</div>
                    <div class="text-gray-600 dark:text-gray-400 text-sm font-medium mt-1">Daraja</div>
                </div>
            </div>
                        ...
                        >
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                <span class="bg-white/20 px-2 py-0.5 rounded text-white/90 text-xs font-medium">
                                    {{ level.numbers_count }} raqam
                                </span>
                                <span class="text-white/70 text-xs">⏱ {{ level.time_per_number }}s</span>
                            </div>
            ...
            <div v-if="achievements.length" class="mb-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div
                        ...
                        :key="achievement.id"
                        class="bg-white dark:bg-gray-800 rounded-2xl p-4 text-center border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all"
            ...
            <!-- Number Master Ranks -->
            <div v-if="numberMasterRanks.length" class="mb-8">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>👑</span> Raqam Ustasi Darajalari
                </h2>
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 shadow-lg">
                    <div class="flex overflow-x-auto gap-4 pb-2">
                        <div
                            v-for="rank in numberMasterRanks"
                            :key="rank.id"
                            class="flex-shrink-0 text-center p-3 rounded-xl min-w-[120px]"
                            :class="[
                                currentRank?.id === rank.id
                                    ? 'bg-cyan-500/20 ring-2 ring-cyan-400 dark:bg-cyan-500/30'
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-4 relative animate-scale-in">
                    ...
                    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-5 rounded-t-2xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-xl font-black text-white">Number Listener</h2>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                            ...
                            <p class="text-green-700 dark:text-green-300 text-sm">
                                Ovoz <strong>yordamida eshitilgan raqamlarni</strong> to'g'ri yozing! Qanchalik aniq va tez yozsangiz, shunchalik ko'p ball olasiz.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2.5">
                                    ...
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Raqamni diqqat bilan tinglang</p>
                                </div>
                                <div class="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-blue-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">2</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Eshitgan raqamni input maydoniga yozing</p>
                                </div>
                                <div class="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2.5">
                                ...
                                <div class="flex items-center gap-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-blue-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">4</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Keyin keyingi raqamga o'ting</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 border border-yellow-200 dark:border-yellow-800">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                ...
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                                    ...
                                    <span class="text-gray-600 dark:text-gray-400">Min raqam</span>
                                </div>
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                        ...
                        <button
                            @click="showTutorial = false"
                            class="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl hover:shadow-lg transition-all text-sm"
                        >
                            Tushundim! 🚀
                        </button>
                    </div>
