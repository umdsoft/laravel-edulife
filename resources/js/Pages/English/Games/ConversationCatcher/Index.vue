...
const filteredLevels = computed(() => {
    ...
    return props.levels.filter(level =>
        level.categories?.includes(selectedCategory.value) ||
...
const currentRank = computed(() => {
    if (!props.conversationMasterRanks.length) return null;

    let current = props.conversationMasterRanks[0];
    for (const rank of props.conversationMasterRanks) {
...
const tips = [
    { icon: '💡', text: "Kontekstga qarab to'g'ri javobni tanlang" },
    ...
    <StudentLayout>
        <div class="min-h-screen pb-8">
            ...
            <div class="relative rounded-3xl p-6 md:p-8 mb-8 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-gradient-to-br from-fuchsia-600 via-pink-600 to-rose-500 animate-gradient"></div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-pink-400 via-rose-400 to-fuchsia-500 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shadow-2xl shadow-pink-500/40 transform hover:scale-105 hover:rotate-3 transition-all duration-300">
                                    💬
                                </div>
                            </div>
                            <div>
                                <h1 class="text-3xl md:text-5xl font-black text-white mb-1 tracking-tight">
                                    Conversation <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">Catcher</span>
                                </h1>
                                <p class="text-white/80 text-sm md:text-lg font-medium">
                                    Suhbatlarni <span class="text-yellow-300 font-bold">to'ldiring</span> va <span class="text-green-400 font-bold">dialoglarni yakunlang</span>!
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 grid grid-cols-2 md:grid-cols-5 gap-3">
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-2xl font-black text-white">{{ stats.games_played || 0 }}</p>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-2xl font-black text-white">{{ stats.correct_answers || 0 }}</p>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-2xl font-black text-white">{{ stats.accuracy || 0 }}%</p>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-amber-400 to-amber-600 rounded-xl flex items-center justify-center text-xl shadow-lg transition-transform group-hover:scale-110">
                                    🔥
                                </div>
                                <div>
                                    <p class="text-2xl font-black text-white">{{ stats.best_streak || 0 }}</p>
                                    <p class="text-white/60 text-xs">Streak</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-base font-black text-white leading-tight">{{ currentRank?.name || 'Yangi' }}</p>
            ...
            <!-- Game Modes Section -->
            <div v-if="gameModes.length > 0" class="mb-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                    <div
                        ...
                        :key="mode.id"
                        class="bg-white dark:bg-gray-800 rounded-xl p-4 cursor-pointer transform hover:scale-105 transition-all shadow-md hover:shadow-lg border border-gray-200 dark:border-gray-700"
                        :class="[
                            selectedMode === mode.id
                                ? 'ring-4 ring-pink-400 dark:ring-pink-600'
                                : ''
                        ]"
            ...
            <div v-if="categories.length > 0" class="mb-8">
                ...
                <div class="flex flex-wrap gap-3">
                    <button
                        ...
                        :key="category.id"
                        class="px-4 py-2 rounded-xl font-medium transition-all shadow-sm hover:shadow-md"
                        :class="[
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
                    ...
                    >
                        ...
                        <Link
                            v-if="level.unlocked"
                            :href="route('student.english.games.conversation-catcher.play', { level: level.level_number })"
                        ...
                        >
                            ...
                            <!-- Best Accuracy -->
                            <div v-if="level.best_accuracy" class="text-white/80 text-xs">
                                🎯 {{ level.best_accuracy }}%
                            </div>
                        </Link>
            ...
            <div v-if="achievements.length > 0" class="mb-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div
                        ...
                        :key="achievement.id"
                        class="bg-white dark:bg-gray-800 rounded-xl p-4 text-center shadow-md border border-gray-200 dark:border-gray-700 transition-all hover:scale-105"
                    ...
                    >
                        ...
                        <div v-if="achievement.unlocked" class="mt-2">
                            <span class="text-xs bg-green-500/20 text-green-600 dark:text-green-400 px-2 py-1 rounded-full">
                                Ochildi!
                            </span>
                        </div>
            ...
            <!-- Conversation Master Ranks -->
            <div v-if="conversationMasterRanks.length > 0" class="mb-8">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>👑</span> Suhbat Ustasi Darajalari
                </h2>
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-md border border-gray-200 dark:border-gray-700">
                    <div class="flex overflow-x-auto gap-4 pb-2">
                        <div
                            v-for="rank in conversationMasterRanks"
                            :key="rank.id"
                            class="flex-shrink-0 text-center p-3 rounded-xl min-w-[120px] transition-all"
                            :class="[
                                currentRank?.id === rank.id
                                    ? 'bg-gradient-to-br from-pink-500 to-rose-500 text-white ring-2 ring-pink-400'
                                    : stats.total_xp >= rank.xp_required
                                        ? 'bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/30 dark:to-emerald-900/30 text-gray-900 dark:text-white'
                                        : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white'
                            ]"
                        >
                            <div class="text-2xl mb-1">{{ rank.icon }}</div>
                            <div class="font-medium text-sm">{{ rank.name }}</div>
                            <div class="text-xs" :class="currentRank?.id === rank.id ? 'text-white/80' : 'text-gray-600 dark:text-gray-400'">{{ rank.xp_required }} XP</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <Teleport to="body">
            ...
            >
                <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-4 relative animate-scale-in">
                    ...
                    <!-- Header -->
                    <div class="bg-gradient-to-r from-fuchsia-600 to-pink-600 p-5 rounded-t-2xl">
                        <div class="flex items-center gap-3">
                            <span class="text-3xl">💬</span>
                            <div>
                                <h2 class="text-xl font-black text-white">Conversation Catcher</h2>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                            ...
                            <p class="text-green-700 dark:text-green-300 text-sm">
                                Suhbatdagi <strong>bo'sh joylarni to'ldiring</strong>! Kontekstga qarab to'g'ri javobni tanlang.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    ...
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Dialogni o'qing va kontekstni tushuning</p>
                                </div>
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-pink-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">2</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Variantlardan to'g'risini tanlang</p>
                                </div>
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-pink-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">3</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Barcha dialoglarni to'ldiring</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <button
                            @click="showTutorial = false"
                            class="w-full py-3 bg-gradient-to-r from-fuchsia-600 to-pink-600 text-white font-bold rounded-xl hover:shadow-lg transition-all text-sm"
                        >
                            Tushundim! 🚀
                        </button>
                    </div>
