...
const props = defineProps({
    ...
    spellingMasterRanks: { type: Array, default: () => [] }
});
...
const currentRank = computed(() => {
    if (!props.spellingMasterRanks.length) return null;
    let current = props.spellingMasterRanks[0];
    for (const rank of props.spellingMasterRanks) {
...
const maxStars = computed(() => props.levels?.length * 3 || 45);
...
const getGradientStyle = (index) => {
    const color = getLevelGradient(index);
    return `linear-gradient(135deg, ${color}, ${adjustColor(color, -40)})`;
};

const tips = [
    { icon: '🎯', text: "So'zlarni diqqat bilan tinglang va to'g'ri yozing" },
    { icon: '⭐', text: "3 ta yulduz olish uchun 95%+ aniqlik kerak" },
    { icon: '🔥', text: "Har kuni mashq qilsangiz, imloingiz yaxshilanadi!" },
];
</script>

<template>
    <Head title="Spelling Bee - Levels" />
    ...
    <StudentLayout>
        <div class="min-h-screen pb-8">
            ...
            <div class="relative rounded-3xl p-6 md:p-8 mb-8 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-gradient-to-br from-yellow-500 via-amber-500 to-orange-500 animate-gradient"></div>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="relative w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-yellow-400 via-amber-400 to-orange-500 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shadow-2xl shadow-amber-500/40 transform hover:scale-105 hover:rotate-3 transition-all duration-300">
                                    🐝
                                </div>
                            </div>
                            <div>
                                <h1 class="text-3xl md:text-5xl font-black text-white mb-1 tracking-tight">
                                    Spelling <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-200 to-amber-200">Bee</span>
                                </h1>
                                <p class="text-white/80 text-sm md:text-lg font-medium">
                                    So'zlarni <span class="text-yellow-200 font-bold">to'g'ri</span> va <span class="text-amber-200 font-bold">aniq</span> yozing!
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-lg font-black text-white">{{ currentRank?.name || 'Yangi' }}</div>
            ...
            <div v-if="gameModes && gameModes.length" class="mb-8 bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <div
                        ...
                        :key="mode.id"
                        class="p-4 rounded-xl cursor-pointer transform hover:scale-105 transition-all border-2"
                        :class="[
                            selectedMode === mode.id
                                ? 'bg-gradient-to-br from-amber-500 to-orange-500 border-amber-400 text-white shadow-lg'
                                : 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 hover:border-amber-400 dark:hover:border-amber-500'
                        ]"
                    ...
                    >
                        ...
                        <div
                            ...
                            :class="selectedMode === mode.id ? 'text-white' : 'text-gray-900 dark:text-white'"
                        >
                            {{ mode.name }}
                        </div>
                        <div
                            class="text-xs mt-1 text-center"
                            :class="selectedMode === mode.id ? 'text-white/90' : 'text-gray-500 dark:text-gray-400'"
                        >
                            {{ mode.description }}
                        </div>
                    </div>
            ...
            <div v-if="categories && categories.length" class="mb-8 bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-200 dark:border-gray-700">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    ...
                    <button
                        ...
                        :class="[
                            selectedCategory === category.id
                                ? 'bg-gradient-to-br from-amber-500 to-orange-500 border-amber-400 text-white shadow-lg'
                                : 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white hover:border-amber-400 dark:hover:border-amber-500'
                        ]"
            ...
            <div class="mb-8">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>🎯</span> Darajalar
                    <span class="text-sm font-normal text-gray-500">({{ filteredLevels?.length || 15 }} ta)</span>
                </h2>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
                    ...
                    >
                        ...
                        <Link
                            v-if="level.unlocked"
                            :href="route('student.english.games.spelling-bee.play', { level: level.level_number })"
                        ...
                        >
                            ...
                            <!-- Words Count -->
                            <div class="bg-white/20 px-2 py-0.5 rounded text-white/90 text-xs font-medium mb-2">
                        ...
                        >
                            ...
                            <!-- Level Name -->
                            <div class="text-gray-500 dark:text-gray-400 text-xs text-center">{{ level.name }}</div>
            ...
            <div v-if="achievements && achievements.length" class="mb-8 bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-200 dark:border-gray-700">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>🏆</span> Yutuqlar
                </h2>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div
                        ...
                        :key="achievement.id"
                        class="rounded-xl p-4 text-center border-2 transition-all"
                    ...
                    >
                        ...
                        <div
                            ...
                            :class="achievement.unlocked ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'"
                        >
                            {{ achievement.name }}
                        </div>
                        <div
                            class="text-xs mt-1"
                            :class="achievement.unlocked ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'"
                        >
            ...
            <!-- Ranks Section (if available) -->
            <div v-if="spellingMasterRanks && spellingMasterRanks.length" class="mb-8 bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-lg border border-gray-200 dark:border-gray-700">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>👑</span> Imlo Darajalari
                </h2>
                <div class="flex overflow-x-auto gap-4 pb-2">
                    <div
                        v-for="rank in spellingMasterRanks"
                        :key="rank.id"
                        class="flex-shrink-0 text-center p-3 rounded-xl min-w-[100px] border-2 transition-all"
                        :class="[
                    ...
                    >
                        ...
                        <div
                            class="text-xs"
                            :class="currentRank?.id === rank.id ? 'text-white/90' : 'text-gray-500 dark:text-gray-400'"
                        >
                            {{ rank.xp_required }} XP
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-4 relative animate-scale-in">
                    ...
                    <div class="bg-gradient-to-r from-amber-600 to-orange-600 p-5 rounded-t-2xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-xl font-black text-white">Spelling Bee</h2>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                            ...
                            <p class="text-green-700 dark:text-green-300 text-sm">
                                <strong>So'zlarni to'g'ri yozing!</strong> Har bir so'zni diqqat bilan eshitib va ko'rib, to'g'ri imlo bilan yozing.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5">
                                    ...
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">So'zni diqqat bilan eshiting yoki ko'ring</p>
                                </div>
                                <div class="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-amber-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">2</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Input maydoniga to'g'ri yozing</p>
                                </div>
                                <div class="flex items-center gap-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5">
                        ...
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 border border-yellow-200 dark:border-yellow-800">
                            ...
                            <div class="grid grid-cols-2 gap-2 text-xs">
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                                    <span class="text-gray-700 dark:text-gray-300">To'g'ri imlo:</span>
