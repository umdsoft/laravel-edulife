...
const currentRank = computed(() => {
    if (!props.memoryMasterRanks.length) return null;

    let current = props.memoryMasterRanks[0];
    for (const rank of props.memoryMasterRanks) {
...
const tips = [
    { icon: '🧠', text: "So'zlarni ko'rib eslashga harakat qiling" },
    { icon: '⭐', text: "3 ta yulduz olish uchun barcha so'zlarni to'g'ri eslab qoling" },
    { icon: '🎯', text: "Qiyinroq darajalar ko'proq mukofot beradi" },
];
</script>

<template>
    <Head title="Word Recall - Levels" />
    ...
    <StudentLayout>
        <div class="min-h-screen pb-8">
            ...
            <div class="relative rounded-3xl p-6 md:p-8 mb-8 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-gradient-to-br from-pink-600 via-rose-600 to-red-500 animate-gradient"></div>
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        ...
                        <div class="flex items-center gap-4">
                            <!-- Animated Icon -->
                            <div class="relative">
                                <div class="absolute inset-0 bg-pink-400/30 rounded-2xl blur-xl animate-pulse"></div>
                                <div class="relative w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-pink-400 via-rose-400 to-red-500 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shadow-2xl shadow-rose-500/40 transform hover:scale-105 hover:rotate-3 transition-all duration-300">
                                    🧠
                                </div>
                            </div>
                            <div>
                                <h1 class="text-3xl md:text-5xl font-black text-white mb-1 tracking-tight">
                                    Word <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-rose-200">Recall</span>
                                </h1>
                                <p class="text-white/80 text-sm md:text-lg font-medium">
                                    So'zlarni <span class="text-pink-300 font-bold">eslab</span> qoling va <span class="text-yellow-300 font-bold">tartibda</span> yozing!
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-3">
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-2xl font-black text-white">{{ stats.games_played || 0 }}</div>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-2xl font-black text-white">{{ stats.accuracy || 0 }}%</div>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center text-xl shadow-lg transition-transform group-hover:scale-110">
                                    🔥
                                </div>
                                <div>
                                    <div class="text-2xl font-black text-white">{{ stats.best_streak || 0 }}</div>
                                    <p class="text-white/60 text-xs">Eng yaxshi seriya</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="group bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all hover:scale-[1.02]">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-xl font-black text-white">{{ currentRank?.name || 'Yangi' }}</div>
            ...
            <!-- Game Modes Section (White Card) -->
            <div v-if="gameModes && gameModes.length > 0" class="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 mb-6 shadow-lg">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
                    <div
                        ...
                        :key="mode.id"
                        class="bg-gradient-to-br rounded-xl p-4 cursor-pointer transform hover:scale-105 transition-all border-2"
                        :class="[
                            selectedMode === mode.id
                                ? 'border-pink-500 from-pink-50 to-rose-50 dark:from-pink-900/20 dark:to-rose-900/20 shadow-lg shadow-pink-500/20'
                                : 'border-gray-200 dark:border-gray-700 from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 hover:border-pink-300 dark:hover:border-pink-600'
                        ]"
                        @click="selectedMode = mode.id"
                    >
                        <div class="text-2xl mb-2 text-center">{{ mode.icon }}</div>
                        <div class="font-semibold text-center text-sm" :class="selectedMode === mode.id ? 'text-pink-700 dark:text-pink-300' : 'text-gray-700 dark:text-gray-300'">{{ mode.name }}</div>
                        <div class="text-xs mt-1 text-center" :class="selectedMode === mode.id ? 'text-pink-600 dark:text-pink-400' : 'text-gray-500 dark:text-gray-400'">{{ mode.description }}</div>
                    </div>
                </div>
            </div>

            <!-- Categories Section (White Card) -->
            <div v-if="categories && categories.length > 0" class="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 mb-6 shadow-lg">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                <div class="flex flex-wrap gap-3">
                    <button
                        ...
                        :key="category.id"
                        class="px-4 py-2 rounded-xl font-medium transition-all border-2"
                        :class="[
                            selectedCategory === category.id
                                ? 'bg-pink-500 text-white border-pink-600 shadow-lg shadow-pink-500/30'
                                : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600 hover:border-pink-400 dark:hover:border-pink-500'
                        ]"
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
                    ...
                    >
                        ...
                        <Link
                            v-if="level.unlocked"
                            :href="route('student.english.games.word-recall.play', { level: level.level_number })"
                        ...
                        >
                            ...
                            <!-- Level Name -->
                            <div class="text-white font-bold text-center text-sm md:text-base mb-2">
                                {{ level.name }}
                            </div>
                            ...
                            <div class="flex flex-col items-center gap-1 mb-2">
                                <div class="flex items-center gap-2 text-xs text-white/80">
                                    <span>{{ level.words_per_round }} so'z</span>
                                    <span>•</span>
                                    <span>{{ level.rounds }} raund</span>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <!-- Level Name -->
                            <div class="text-gray-500 dark:text-gray-400 text-sm mb-2">{{ level.name }}</div>
            ...
            <!-- Achievements Section (White Card) -->
            <div v-if="achievements && achievements.length > 0" class="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 mb-6 shadow-lg">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>🏆</span> Yutuqlar
                </h2>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div
                        ...
                        :key="achievement.id"
                        class="bg-gradient-to-br rounded-xl p-4 text-center border-2 transition-all hover:scale-105"
                    ...
                    >
                        ...
                        <div class="font-medium text-sm" :class="achievement.unlocked ? 'text-green-800 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'">
                            {{ achievement.name }}
                        </div>
                        <div class="text-xs mt-1" :class="achievement.unlocked ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'">
                        ...
                        <div v-else class="mt-2">
                            <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5 mb-1">
                                <div class="bg-pink-500 h-1.5 rounded-full" :style="{ width: (achievement.progress || 0) + '%' }"></div>
                            </div>
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ achievement.progress || 0 }}%</span>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Memory Master Ranks (White Card) -->
            <div v-if="memoryMasterRanks && memoryMasterRanks.length > 0" class="bg-white dark:bg-gray-800 rounded-2xl p-5 md:p-6 mb-6 shadow-lg">
                <h2 class="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    <span>👑</span> Xotira Ustasi Darajalari
                </h2>
                <div class="flex overflow-x-auto gap-4 pb-2">
                    <div
                        v-for="rank in memoryMasterRanks"
                        :key="rank.id"
                        class="flex-shrink-0 text-center p-4 rounded-xl min-w-[120px] transition-all border-2"
                        :class="[
                            currentRank?.id === rank.id
                                ? 'bg-gradient-to-br from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 border-purple-400 dark:border-purple-500 shadow-lg'
                    ...
                    >
                        ...
                        <div class="font-medium text-sm" :class="[
                            currentRank?.id === rank.id ? 'text-purple-800 dark:text-purple-300' :
                            stats.total_xp >= rank.xp_required ? 'text-green-800 dark:text-green-300' :
                            'text-gray-700 dark:text-gray-300'
                        ]">
                            {{ rank.name }}
                        </div>
                        <div class="text-xs mt-1" :class="[
                            currentRank?.id === rank.id ? 'text-purple-600 dark:text-purple-400' :
                            stats.total_xp >= rank.xp_required ? 'text-green-600 dark:text-green-400' :
            ...
            <!-- Tips Section -->
            <div class="bg-gradient-to-r from-pink-50 to-rose-50 dark:from-pink-900/20 dark:to-rose-900/20 rounded-2xl p-5 border border-pink-200 dark:border-pink-800">
                <h3 class="font-bold text-pink-900 dark:text-pink-300 mb-3 flex items-center gap-2">
        ...
        <Teleport to="body">
            ...
            >
                <div class="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-4 relative animate-scale-in">
                    ...
                    <!-- Header -->
                    <div class="bg-gradient-to-r from-pink-600 to-rose-600 p-5 rounded-t-2xl">
                        <div class="flex items-center gap-3">
                            <span class="text-3xl">🧠</span>
                            <div>
                                <h2 class="text-xl font-black text-white">Word Recall</h2>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                            ...
                            <p class="text-green-700 dark:text-green-300 text-sm">
                                Ko'rsatilgan <strong>so'zlarni eslab qoling</strong> va keyin ularni to'g'ri tartibda yozing. Xotira qobiliyatingizni sinang!
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-pink-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">1</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">So'zlarni diqqat bilan ko'ring</p>
                                </div>
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-pink-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">2</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Eslab qolishga harakat qiling</p>
                                </div>
                                <div class="flex items-center gap-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg p-2.5">
                                    <div class="w-6 h-6 bg-pink-500 text-white text-xs font-bold rounded flex items-center justify-center flex-shrink-0">3</div>
                                    <p class="text-gray-800 dark:text-gray-200 text-sm">Ularni to'g'ri tartibda yozing</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4 border border-yellow-200 dark:border-yellow-800">
                            ...
                            <div class="space-y-2 text-xs">
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                                    ...
                                    <span class="font-bold text-yellow-600">50%+ to'g'ri</span>
                                </div>
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                                    <span class="text-gray-700 dark:text-gray-300">2 yulduz:</span>
                                    <span class="font-bold text-yellow-600">75%+ to'g'ri</span>
                                </div>
                                <div class="flex justify-between items-center bg-white/50 dark:bg-black/20 rounded p-2">
                                    <span class="text-gray-700 dark:text-gray-300">3 yulduz:</span>
                                    <span class="font-bold text-yellow-600">100% to'g'ri</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <button
                            @click="showTutorial = false"
                            class="w-full py-3 bg-gradient-to-r from-pink-600 to-rose-600 text-white font-bold rounded-xl hover:shadow-lg transition-all text-sm"
                        >
                            Tushundim! 🚀
                        </button>
                    </div>
