...
<template>
    ...
    <Head title="Ingliz tili - Reyting" />
    ...
    <StudentLayout>
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <div class="text-center">
                ...
                <p class="text-gray-600 dark:text-gray-400">Butun dunyo o'quvchilari bilan raqobatlashing</p>
            </div>
            ...
            <div class="flex space-x-2 mb-8 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
                <button v-for="period in periods" :key="period.id" @click="selectedPeriod = period.id"
                    class="flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors" :class="selectedPeriod === period.id
            ...
            <!-- User's Current Rank -->
            <div v-if="currentUserRank"
                class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-2xl p-4 mb-6 text-white">
            ...
            <div v-if="currentLeaderboard.length >= 3" class="flex items-end justify-center space-x-4 mb-8">
                ...
                <div class="flex flex-col items-center">
                    <div
                        class="w-16 h-16 rounded-full bg-gradient-to-br from-gray-300 to-gray-500 flex items-center justify-center text-xl font-bold text-white mb-2 shadow-lg">
                        {{ currentLeaderboard[1]?.user?.name?.[0]?.toUpperCase() }}
                    </div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-20">{{
                        currentLeaderboard[1]?.user?.name }}</p>
                    <p class="text-xs text-gray-500">{{ getValueLabel(currentLeaderboard[1]) }}</p>
                    <div
                        class="w-20 h-16 bg-gradient-to-t from-gray-300 to-gray-400 rounded-t-lg mt-2 flex items-center justify-center">
                ...
                <div class="flex flex-col items-center -mt-4">
                    <div
                        class="w-20 h-20 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center text-2xl font-bold text-white mb-2 shadow-lg ring-4 ring-yellow-300">
                        {{ currentLeaderboard[0]?.user?.name?.[0]?.toUpperCase() }}
                    </div>
                    <p class="font-medium text-gray-900 dark:text-white truncate max-w-24">{{
                        currentLeaderboard[0]?.user?.name }}</p>
                    <p class="text-sm text-gray-500">{{ getValueLabel(currentLeaderboard[0]) }}</p>
                    <div
                        class="w-24 h-24 bg-gradient-to-t from-yellow-400 to-yellow-500 rounded-t-lg mt-2 flex items-center justify-center">
                ...
                <div class="flex flex-col items-center">
                    <div
                        class="w-16 h-16 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-xl font-bold text-white mb-2 shadow-lg">
                        {{ currentLeaderboard[2]?.user?.name?.[0]?.toUpperCase() }}
                    </div>
                    <p class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-20">{{
                        currentLeaderboard[2]?.user?.name }}</p>
                    <p class="text-xs text-gray-500">{{ getValueLabel(currentLeaderboard[2]) }}</p>
                    <div
                        class="w-20 h-12 bg-gradient-to-t from-orange-400 to-orange-500 rounded-t-lg mt-2 flex items-center justify-center">
                        <span class="text-2xl">🥉</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden">
                <div v-for="(entry, index) in currentLeaderboard.slice(3)" :key="entry.user_id"
                    ...
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-medium mx-3">
